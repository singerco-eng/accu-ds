...
export default function AvatarsPage() {
  return (
    <div>
      <h1 className="accu-text-display-sm font-bold mb-6">Avatars</h1>
      <div className="grid gap-8">
        <section>
          <h2 className="accu-text-display-sm font-bold mb-3">Image Variants</h2>
          <div className="flex flex-wrap items-end gap-4">
            {sizes.map((size) => (
              <div key={`img-${size}`} className="flex flex-col items-center gap-2">
                <Avatar size={size} src={imageUrl} alt={`Avatar ${size}`} />
                <span className="accu-text-body-sm font-normal">{size}px</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section>
          <h2 className="accu-text-display-sm font-bold mb-3">Initials Variants</h2>
          <div className="flex flex-wrap items-end gap-4">
            {sizes.map((size) => (
